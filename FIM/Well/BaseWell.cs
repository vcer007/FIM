        ...
        private void UpdateDerivatives(BaseBlock block)
        {
            ...
            if (control == Global.WellControl.OilRate)
            {
                if (type == Global.WellType.Production)
                {
                    ...
                    dq_solution_gas_dSw = (block.Rso[1] * /*q_oil[1]*/CalculateFlowRate(block.P[1], temp, this.data.scal.GetKro(block.Sg[1], block.Sw[2], data.pvt.connateWaterSaturation), block.viscosityOil[2], WI, block.Bo[2]) - block.Rso[1] * q_oil[1]) / Global.EPSILON_S;
